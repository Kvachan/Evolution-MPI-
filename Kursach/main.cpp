...
static std::string PREFS_KEY_BRAINS_DIRECTORY = "BrainsDirectory";
static GeneticAlgorithm<OptimizableNeuralNetwork, double> *ga;
static AgentsEnvironment *environment;
static int populationNumer = 0;
static bool play = true;
static bool staticFood = true;
static bool regenerationFood = true;

void initializeAddingFoodFunctionality() {
...
//        });
//    }
void update(GeneticAlgorithm<OptimizableNeuralNetwork, double> *ga) {
    OptimizableNeuralNetwork *bestBrain = ga->getBest();
    double fit = *(ga->fitness(bestBrain));
    ga->clearCache();
}

void addGASystemOutIterationListener() {
    IterartionListener<OptimizableNeuralNetwork, double> *buf = new IterartionListener<OptimizableNeuralNetwork, double>();
    ga->addIterationListener(buf);
    update(ga);
}

void setAgentBrains(NeuralNetwork *newBrain) {
...
void initializeAgents(NeuralNetwork *brain, int agentsCount) {

    int environmentWidth = environment->getWidth();
    int environmentHeight = environment->getHeight();

    for (int i = 0; i < agentsCount; i++) {
        int x = rand() % environmentWidth;
        int y = rand() % environmentHeight;
        double direction = rand() * 2 * M_PI;
...
Food* createRandomFood(int width, int height) {
    int x = rand() % width;
    int y = rand() % height;
    ...
    if (staticFood) {
        ...
        double speed = fRand(std::numeric_limits<double>::min(), std::numeric_limits<double>::max()) * 2;//do
        double direction = fRand(std::numeric_limits<double>::min(), std::numeric_limits<double>::max()) * 2 * M_PI;
...
void initializeGeneticAlgorithm(int populationSize, int parentalChromosomesSurviveCount, OptimizableNeuralNetwork* baseNeuralNetwork) {
    Population<OptimizableNeuralNetwork, double> *brains = new Population<OptimizableNeuralNetwork, double>();

    for (int i = 0; i < (populationSize - 1); i++) {
        if (baseNeuralNetwork == nullptr) {
    ...
    TournamentEnvironmentFitness *fit = new TournamentEnvironmentFitness();
...
void initializeEnvironment(int environmentWidth, int environmentHeight, int agentsCount, int foodCount) {
    environment = new AgentsEnvironment(environmentWidth, environmentHeight);
    EaternFoodObserver *buf = new EaternFoodObserver();
    environment->addListener(buf);
    addRandomPieceOfFood(environment);
}
...
int main(int argc, char *argv[])
{
    ...
    int agentsCount = 15;
    int foodCount = 10;
    initializeGeneticAlgorithm(gaPopulationSize, parentalChromosomesSurviveCount, nullptr);
